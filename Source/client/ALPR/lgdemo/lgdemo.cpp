...
static Mode GetVideoMode(const char* param_mode);
...
static Mode GetVideoMode(const char* param_mode)
{
    ...
    if (strncmp(param_mode, "camera", 10) == 0) {
        mode = Mode::mLive_Video;
    }
    else if (strncmp(param_mode, "video", 10) == 0) {
        mode = Mode::mPlayback_Video;
    }
    else if (strncmp(param_mode, "image", 10) == 0) {
        mode = Mode::mImage_File;
    }
    else {
        mode = Mode::mNone;
    }
...
static int GetVideoDevice(void)
{
    ...
    if (numdev == 0) {
        std::cout << "no available Camera" << std::endl;
        exit(0);
    }
#if 0
        std::cout << "E - exit" << std::endl;
...
static VideoResolution GetVideoResolution(void)
{
    ...
    return(vres);
#endif
    return(VideoResolution::r640X480);
}
/***********************************************************************************/
